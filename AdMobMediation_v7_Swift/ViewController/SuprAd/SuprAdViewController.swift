...
class SuprAdViewController: UIViewController {
    
    @IBOutlet weak var suprAdTableView: UITableView!
    
    private let TestSuprAdUnit: String = "Your SuprAd ad unit"
    
    private let googleMediationSuprAdPosition:   Int = 8
    
    private var refreshControl: UIRefreshControl?
    private var adLoader: GADAdLoader?
    private var gADUnifiedSuprAd:   GADUnifiedNativeAd? // SuprAd
...
extension SuprAdViewController {
    ...
    private func setupGADAdloader() {
        ...
        adLoader = GADAdLoader.init(adUnitID: TestSuprAdUnit, rootViewController: self, adTypes: [.unifiedNative], options: [])
            ...
            if let gADUnifiedSuprAd = gADUnifiedSuprAd,
               let extraAssets = gADUnifiedSuprAd.extraAssets {
                if extraAssets.keys.contains("adSizeWidth") &&
                    ...
                    trekSuprAdTableViewCell.setGADUnifiedNativeAdData(nativeAd: gADUnifiedSuprAd, size: preferedMediaViewSize)
...
extension SuprAdViewController: GADUnifiedNativeAdLoaderDelegate {
